...
const MonsterPreview = ({ data }) => {
  const {
    ...
    armorClass, hitPoints, hitDice,
    speed, abilityScores,
    savingThrows, skills,
    damageVulnerabilities, damageResistances, damageImmunities, conditionImmunities,
    senses, languages, challengeRating, proficiencyBonus,
    traits, actions, reactions, legendaryActions, legendaryResistances, lairActions, regionalEffects,
    description, image, extraSections
  } = data;
      ...
      >
        {image && (
          <div className="monster-image-wrapper">
            <img src={image} alt={`${name} art`} className="monster-image" />
          </div>
        )}
        ...
        <div className="monster-statblock">
          ...
          <div><strong>Speed:</strong> {Object.entries(speed || {}).map(([k, v]) => `${k}: ${v}`).join(', ')}</div>
          <div className="monster-section">
        ...
        {damageVulnerabilities?.length > 0 && (
          <p><strong>Vulnerabilities:</strong> {damageVulnerabilities.join(', ')}</p>
        )}
        {damageResistances?.length > 0 && (
          <p><strong>Resistances:</strong> {damageResistances.join(', ')}</p>
        )}
        {damageImmunities?.length > 0 && (
          <p><strong>Immunities:</strong> {damageImmunities.join(', ')}</p>
        )}
        ...
        {Object.keys(senses).length > 0 && (
          <p><strong>Senses:</strong> {Object.entries(senses).map(([k, v]) => `${k}: ${v}`).join(', ')}</p>
        )}
