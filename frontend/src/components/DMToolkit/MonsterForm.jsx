...
const MonsterForm = ({ monster, setMonster, closeForm, onSubmit }) => {
  ...
  return (
    <div className="monster-form-panel">
      ...
      {/* Basic Info */}
      {['name', 'size', 'type', 'alignment', 'armorClass', 'hitPoints', 'hitDice'].map(field => (
        <div key={field}>
          <label>{field.replace(/([A-Z])/g, ' $1')}</label>
          <input value={monster[field]} onChange={e => handleChange(field, e.target.value)} />
        </div>
      ))}
      ...
      {Object.keys(monster.abilityScores).map(attr => (
        <div key={attr}>
          ...
          <input
            ...
            onChange={e => handleNestedChange('abilityScores', attr, e.target.value)}
          />
        </div>
      ))}
      ...
      {Object.entries(monster.savingThrows || {}).map(([key, val]) => (
        <div key={key}>
          ...
          <input
            value={val}
            onChange={e => handleKeyValueChange('savingThrows', key, e.target.value)}
          />
          <button onClick={() => removeKeyFromSection('savingThrows', key)} className="remove-btn">Remove</button>
        </div>
      ))}
      <div className="key-pair-row">
        <input
          ...
          value={newSave.key}
          onChange={e => setNewSave(prev => ({ ...prev, key: e.target.value }))}
        />
        <input
          type="text"
          placeholder="Value (e.g. +5)"
          value={newSave.value}
          onChange={e => setNewSave(prev => ({ ...prev, value: e.target.value }))}
        />
        <button
          onClick={() => {
      ...
      {Object.entries(monster.skills || {}).map(([key, val]) => (
        <div key={key}>
          ...
          <input
            value={val}
            onChange={e => handleKeyValueChange('skills', key, e.target.value)}
          />
          <button onClick={() => removeKeyFromSection('skills', key)} className="remove-btn">Remove</button>
        </div>
      ))}
      <div className="key-pair-row">
        <input
      ...
      {/* Senses */}
      <h3>Senses</h3>
      {['darkvision', 'blindsight', 'tremorsense', 'truesight', 'passivePerception'].map(sense => (
        <div key={sense}>
          <label>{sense}</label>
          <input
            value={monster.senses[sense]}
            onChange={e => handleNestedChange('senses', sense, e.target.value)}
          />
        </div>
      ))}
      ...
      <input value={monster.challengeRating} onChange={e => handleChange('challengeRating', e.target.value)} />

      <label>Proficiency Bonus</label>
      <input value={monster.proficiencyBonus} onChange={e => handleChange('proficiencyBonus', e.target.value)} />
      ...
      {[
        ...
        <div key={section}>
          <h3>{section.replace(/([A-Z])/g, ' $1').replace(/^./, s => s.toUpperCase())}</h3>
          {monster[section]?.map((item, index) => (
            <div key={index} className="custom-section-block">
              <label>Title</label>
              <input
                value={item.name}
                onChange={e => updateListItem(section, index, 'name', e.target.value)}
              />
              <label>Description</label>
              <textarea
                value={item.desc}
                onChange={e => updateListItem(section, index, 'desc', e.target.value)}
              />
              <button onClick={() => removeListItem(section, index)} className="remove-btn">Remove</button>
            </div>
          ))}
          <button className="add-section-btn" onClick={() => addListItem(section)}>+ Add {section}</button>
        </div>
      ))}

      {/* Description & Image */}
      <label>Description (Lore)</label>
      <textarea value={monster.description} onChange={e => handleChange('description', e.target.value)} />
    ...
    <input
      ...
      onChange={async (e) => {
        ...
        if (file) {
          ...
          try {
            const res = await fetch('/api/uploads/monsters', {
              method: 'POST',
              body: formData
            });
            const data = await res.json();
            handleChange('image', data.url); // assume server returns { url: '/uploads/monsters/filename.png' }
          } catch (err) {
