...
const NPCForm = ({ npc, setNPC, closeForm, onSubmit }) => {
  ...
  return (
    <div className="npc-form-panel">
      ...
      {/* General Info */}
      {[
        'name', 'race', 'class', 'gender', 'age', 'alignment', 'background', 'occupation', 'tokenSize',
        'armorClass', 'hitPoints', 'hitDice', 'speed', 'proficiencyBonus', 'challengeRating', 'description'
      ].map(field => (
        <div key={field}>
          <label>{field}</label>
          <input
      ...
      {Object.keys(npc.abilityScores || {}).map(attr => (
        <div key={attr}>
          ...
          <input
            ...
            onChange={e => handleNestedChange('abilityScores', attr, parseInt(e.target.value))}
          />
        </div>
      ))}
      ...
      {Object.entries(npc.savingThrows || {}).map(([key, val]) => (
        <div key={key}>
          <label>{key}</label>
          <input value={val} onChange={e => handleKeyValueChange('savingThrows', key, e.target.value)} />
      ...
      {Object.entries(npc.skills || {}).map(([key, val]) => (
        <div key={key}>
          <label>{key}</label>
          <input value={val} onChange={e => handleKeyValueChange('skills', key, e.target.value)} />
        ...
        <div key={field}>
          ...
          {(npc[field] || []).map((item, i) => (
            <div key={i} className="custom-section-block">
              ...
              />
              <button onClick={() => removeFromList(field, i)} className="remove-btn">Remove</button>
            </div>
          ))}
          <button onClick={() => addToList(field, { name: '', desc: '' })} className="add-section-btn">+ Add {field.slice(0, -1)}</button>
        </div>
      ))}
      ...
      <ul>
        {(npc.languages || []).map((lang, i) => (
          <li key={i}>
            ...
            />
            <button onClick={() => removeFromList('languages', i)} className="remove-btn">Remove</button>
          </li>
        ))}
      </ul>
      <input
      ...
      {/* Submit */}
      <button onClick={handleSubmit} className="add-section-btn">Save NPC</button>
    </div>
  );
};
