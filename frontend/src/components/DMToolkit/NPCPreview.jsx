...
const NPCPreview = ({ data }) => {
  const {
    name, race, class: npcClass, gender, age, alignment, background, occupation, tokenSize,
    armorClass, hitPoints, hitDice, speed, proficiencyBonus, challengeRating,
    languages = [], abilityScores = {}, savingThrows = {}, skills = {},
    senses = {}, damageResistances = [], conditionImmunities = [],
    traits = [], actions = [], spells = {}, description = '', image
  } = data;
    ...
    >
      ...
      <ul>
        {Object.entries(abilityScores).map(([attr, val]) => (
          <li key={attr}><strong>{attr.toUpperCase()}:</strong> {val}</li>
        ))}
      </ul>
      ...
      {damageResistances.length > 0 && (
        <p><strong>Damage Resistances:</strong> {damageResistances.join(', ')}</p>
      )}
      {conditionImmunities.length > 0 && (
        <p><strong>Condition Immunities:</strong> {conditionImmunities.join(', ')}</p>
      )}

      {senses?.passivePerception && (
        <p><strong>Passive Perception:</strong> {senses.passivePerception}</p>
      )}
      ...
      {spells.level1?.length > 0 && renderList('1st-Level Spells', spells.level1)}
